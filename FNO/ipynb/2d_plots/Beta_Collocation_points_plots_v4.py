...
REGIME_POSITION_FRAC_MAP = {
    ...
    "wave": {1: (0.25, 0.6), 2: (0.85, 0.6), 3: (0.6, 0.08)},
}

# Boundary style and per-equation parameters
BOUNDARY_LINE_STYLE = {"color": "white", "linewidth": 1.5, "alpha": 0.9, "linestyle": "-"}
BOUNDARY_LINE_STYLE_TRAIN = {"color": "black", "linewidth": 2.5, "alpha": 0.9, "linestyle": "-"}
BOUNDARY_LINE_STYLE_TEST = {"color": "black", "linewidth": 2.5, "alpha": 0.9, "linestyle": "--"}
...
def plot_2d_phase(
    ...
    # current_cmap = "magma"
    # current_cmap = truncated_cmap("viridis_r", minval=0.0, maxval=0.5) #"viridis_r"
    ...
    ylabel = y_label_override if y_label_override is not None else "Collocation Points"
    ...
    if save_path is not None:
        ...
        os.makedirs(save_dir, exist_ok=True)
        out_path = f"{save_dir}/{metric}.pdf"
...
def main():
    ...
    for func, json_file_path in datasets.items():
        ...
        boundary_config = {
            **BOUNDARY_BASE_CONFIG,
            **BOUNDARY_DEFAULT_CONFIG,
            **BOUNDARY_CONFIG_PRESETS.get(func, {}),
        }
