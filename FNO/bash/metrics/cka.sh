                ...
                for j in $(seq $((i+1)) 4)
                    do
                        ...
                        if [ -f "$checkpoint_a" ] && [ -f "$checkpoint_b" ]; then
                            echo "Running CKA similarity analysis: ${config} - seed${seed_a} vs seed${seed_b} - ${kernel} kernel"
                            
                            CUDA_VISIBLE_DEVICES=7 python calculate_cka_similarity.py \
                                --config=$config \
