...
import argparse
def parse_args():
    parser = argparse.ArgumentParser(description='Burgers1D args', add_help=False)
    parser.add_argument("--beta", type=float, default=40)
    parser.add_argument("--n_colloc", type=int, default=20000)
    return parser.parse_known_args()[0]  # Ignore extras
args = parse_args()

class Burgers1D(baseclass.BaseTimePDE):

    def __init__(self, datapath="ref/burgers1d.dat", geom=[-1, 1], time=[0, 1], nu=0.01 / np.pi, beta=40, n_colloc=20000):
        super().__init__()
        self.beta = beta
        ...
        def burger_pde(x, u):
            ...
            return u_t + self.beta * u * u_x - nu * u_xx
    ...
    def training_points(self):
        n = max(1, self.n_colloc // 3)  # Avoid zero points
        n_test = max(1, self.n_colloc // 10)  # Add test points
        return {
            'interior': n,
            'bc': n,
            'ic': n,
