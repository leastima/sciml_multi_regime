...
from multiadam_src.utils import plot

logger = logging.getLogger(__name__)


class ModelSaveCallback(Callback):
    """Callback to save the trained model to a custom location."""
    
    def __init__(self, save_dir, experiment_name, verbose=True):
    ...
    def on_train_begin(self):
        """Debug: Check model save path at training start."""
        if self.verbose:
            print(f"ModelSaveCallback initialized")
            print(f"Target save directory: {os.path.join(self.save_dir, self.experiment_name)}")
            print(f"Model save path: {getattr(self.model, 'model_save_path', 'Not set')}")
        
    def on_train_end(self):
        """Save the trained model directly to our custom directory."""
        if self.verbose:
            print(f"Saving model to custom location: {self.save_dir}")
        
        # Create the target directory
        target_dir = os.path.join(self.save_dir, self.experiment_name)
        os.makedirs(target_dir, exist_ok=True)
        
        # Save the model directly using PyTorch
        try:
            final_epoch = self.model.train_state.epoch
            save_path = os.path.join(target_dir, f"model_epoch_{final_epoch}.pt")
            
            # Create checkpoint with model and optimizer state
            checkpoint = {
                "model_state_dict": self.model.net.state_dict(),
                "train_state": {
                    "epoch": self.model.train_state.epoch,
                    "step": self.model.train_state.step,
                    "best_loss_train": self.model.train_state.best_loss_train,
                    "best_loss_test": self.model.train_state.best_loss_test,
                },
                "loss_history": {
                    "loss_train": self.model.losshistory.loss_train,
                    "loss_test": self.model.losshistory.loss_test,
                    "steps": self.model.losshistory.steps,
                }
            }
            
            # Add optimizer state if available
            if hasattr(self.model, 'opt') and self.model.opt is not None:
                checkpoint["optimizer_state_dict"] = self.model.opt.state_dict()
            ...
            if self.verbose:
                ...
                print(f"Model parameters: {sum(p.numel() for p in self.model.net.parameters())}")
                
        except Exception as e:
            if self.verbose:
                print(f"Error saving model: {e}")
                
            # Fallback: try to copy from original location if it exists
            original_save_path = self.model.model_save_path
            if original_save_path and os.path.exists(original_save_path):
                if self.verbose:
                    print(f"Attempting to copy from original location: {original_save_path}")
                # Look for .pt files (PyTorch models)
                for file in os.listdir(original_save_path):
                    if file.endswith('.pt'):
                        src_file = os.path.join(original_save_path, file)
                        dst_file = os.path.join(target_dir, file)
                        shutil.copy2(src_file, dst_file)
                        if self.verbose:
                            print(f"Model copied to: {dst_file}")
            else:
                if self.verbose:
                    print(f"Warning: No model found at {original_save_path}")
                    print(f"Available attributes in model: {dir(self.model)}")
                    print(f"Model save path: {getattr(self.model, 'model_save_path', 'Not set')}")


class ModelCheckpointCallback(Callback):
    """Callback to save model checkpoints at specific iterations."""
    
    def __init__(self, save_dir, experiment_name, checkpoint_iterations=None, verbose=True):
        super(ModelCheckpointCallback, self).__init__()
        self.save_dir = save_dir
        self.experiment_name = experiment_name
        self.verbose = verbose
        
        # Default checkpoint iterations for analysis
        if checkpoint_iterations is None:
            self.checkpoint_iterations = [2000, 5000, 10000, 15000]  # Key comparison points
        else:
            self.checkpoint_iterations = checkpoint_iterations
            
        # Convert to set for O(1) lookup
        self.checkpoint_set = set(self.checkpoint_iterations)
        
    def on_epoch_end(self):
        """Check if current epoch is a checkpoint and save model."""
        current_epoch = self.model.train_state.epoch
        
        if current_epoch in self.checkpoint_set:
    ...
    def _save_checkpoint(self, epoch):
        ...
        try:
            save_path = os.path.join(target_dir, f"model_checkpoint_epoch_{epoch}.pt")
