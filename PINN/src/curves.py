...
class CurveNet(Module):
    def __init__(self, in_dim, hidden_dim, out_dim, num_layer, curve, architecture, num_bends, fix_start=True,
                 fix_end=True,
                 architecture_kwargs={}):
        ...
        self.net = self.architecture(in_dim, hidden_dim, out_dim, num_layer, fix_points=self.fix_points,
                                     **architecture_kwargs)
        self.curve_modules = []
    ...
    # !!!!!!!!
    def forward(self, pde_x, pde_t, t=None):
        if t is None:
            t = pde_x.data.new(1).uniform_()  # 生成一个与 x 同设备、同数据类型的新张量，并在 [0,1) 范围内随机采样一个值
        coeffs_t = self.coeff_layer(t)
        Coeffs_T.value = coeffs_t

        output = self.net(pde_x, pde_t)
        return output

    def predict(self, pde_x, pde_t, t=None):
        x_res, x_left, x_upper, x_lower = pde_x
        t_res, t_left, t_upper, t_lower = pde_t

        pred_res = self.forward(x_res, t_res, t)
        pred_left = self.forward(x_left, t_left, t)
        pred_upper = self.forward(x_upper, t_upper, t)
        pred_lower = self.forward(x_lower, t_lower, t)
