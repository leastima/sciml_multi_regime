...
from plot_boundary_utils import extract_boundary_polygon, truncated_cmap
# from nys_newton_cg import NysNewtonCG

REGIME_NAMES = {1: "I", 2: "II", 3: "III"}
REGIME_LABEL_STYLE = {"fontsize": 20, "fontweight": "normal", "color": "white", "family": "serif"}
TEXT_PATH_EFFECTS = [PathEffects.withStroke(linewidth=2.5, foreground="black")]
REGIME_POSITION_FRAC_DEFAULT = {1: (0.2, 0.8), 2: (0.8, 0.8), 3: (0.5, 0.08)}
...
def plot_heatmap(
    ...
    regime_overlay=None,
    show_y_axis=True,
    boundary_percentile=70.0,
):
    fig, ax = plt.subplots(figsize=(5, 4))
    mat = np.array(matrix, dtype=float)
    x_idx, y_idx = np.meshgrid(np.arange(len(x_values)), np.arange(len(y_values)))

    mat_filled = np.ma.filled(np.ma.masked_invalid(mat), np.nan)
    if mat_filled.shape[0] > 5 and mat_filled.shape[1] > 5:
        mat_smooth = gaussian_filter(mat_filled, sigma=0.1)
    else:
        mat_smooth = mat_filled
    mat_smooth = np.ma.array(mat_smooth, mask=np.isnan(mat_filled))

    vmin = np.nanmin(mat_filled)
    vmax = np.nanmax(mat_filled)
    if not np.isfinite(vmin) or not np.isfinite(vmax) or vmin == vmax:
        vmin = 0.0 if not np.isfinite(vmin) else vmin
        vmax = vmin + 1e-8

    cmap = truncated_cmap("viridis_r", minval=0.0, maxval=0.8)
    # 1×1 (or thin) grids: contourf needs at least 2×2 — use imshow instead.
    use_imshow = mat_filled.shape[0] < 2 or mat_filled.shape[1] < 2
    if use_imshow:
        mappable = ax.imshow(
            mat_filled,
            origin="lower",
        ...
        collections = getattr(mappable, "collections", None)
        if collections:
    ...
    x_labels = [fmt_tick(v) for v in x_values]
    if len(x_labels) > 10:
        step = int(np.ceil(len(x_labels) / 10))
        x_tick_idx = np.arange(0, len(x_labels), step)
        ax.set_xticks(x_tick_idx)
        ax.set_xticklabels([x_labels[i] for i in x_tick_idx])
    else:
        ax.set_xticklabels(x_labels)

    # Y-axis: ticks at uniform row indices; labels show physical ``y_values``.
    y_values_arr = np.array(y_values, dtype=float)
    y_is_int = np.allclose(y_values_arr, np.round(y_values_arr), atol=1e-6)
    if y_is_int:
        y_labels_full = [str(int(round(v))) for v in y_values_arr]
    else:
        y_labels_full = [fmt_tick(v) for v in y_values_arr]

    max_y_ticks = 8
    if len(y_labels_full) > max_y_ticks:
        step_y = int(np.ceil(len(y_labels_full) / max_y_ticks))
        y_tick_idx = np.arange(0, len(y_labels_full), step_y)
    else:
        y_tick_idx = np.arange(len(y_labels_full))
    ax.set_yticks(y_tick_idx)
    ax.set_yticklabels([y_labels_full[i] for i in y_tick_idx])
    ...
    # Keep axes increasing left->right and bottom->top for interpretability.

    if boundary_data and "training_loss" in boundary_data and "test_error" in boundary_data:
        train_raw = np.array(boundary_data["training_loss"], dtype=float)
        test_raw = np.array(boundary_data["test_error"], dtype=float)
        smooth_sigma = 0.7
        train_smooth = gaussian_filter(train_raw, sigma=smooth_sigma)
        test_smooth = gaussian_filter(test_raw, sigma=smooth_sigma)

        q = float(np.clip(boundary_percentile, 0.0, 100.0))
        train_thresh = np.nanpercentile(train_smooth, q)
        test_thresh = np.nanpercentile(test_smooth, q)
        train_buffer = 0.0
        test_buffer = 0.0

        if np.isfinite(test_thresh) and test_thresh > 0:
            test_masked = np.ma.masked_where(
                train_smooth > (train_thresh + train_buffer), test_smooth
            )
            ax.contour(
                x_idx,
                y_idx,
        ...
        if np.isfinite(train_thresh) and train_thresh > 0:
            ax.contour(
                x_idx,
                y_idx,
                train_smooth,
        ...
        boundary = extract_boundary_polygon(
            mat,
            smoothing_sigma=1.0,
            threshold_ratio=float(np.clip(100.0 - boundary_percentile, 0.0, 100.0)) / 100.0,
            use_binary_mask=False,
        )
        if boundary and boundary.get("polygon") is not None:
            poly = boundary["polygon"]
            ax.plot(poly[:, 1], poly[:, 0], color="white", linewidth=2.5, alpha=0.9)
            if boundary.get("start_point") is not None:
                peak_x, peak_y = boundary["start_point"]
                ax.plot(
                    peak_x,
                    peak_y,
                    marker="D",
    ...
    max_abs = np.nanmax(np.abs([vmin, vmax]))
    ...
    def cb_formatter(val, _pos=None):
        if not np.isfinite(val):
            return ""
        return f"{val / scale:.1f}"

    cbar.formatter = FuncFormatter(cb_formatter)
    cbar.update_ticks()
    cbar.ax.set_title(f"e{exponent:+d}", fontsize=12, pad=2)
...
def plot_training_loss_curves(out_dir: Path) -> None:
    """Plot semilogy and log10 curves from per-case JSON files that contain
    train_*_loss_history (written when running training, not from --load-results)."""
    case_files = sorted(out_dir.glob("b*_hor*.json"))
    series = []
    for path in case_files:
        with path.open("r") as f:
            d = json.load(f)
        h = d.get("train_total_loss_history")
        if not h:
            continue
        y = np.maximum(np.array(h, dtype=float), 1e-30)
        ep = np.arange(1, len(y) + 1)
        sd = d.get("seed")
        sd_lab = f", seed={sd}" if sd is not None else ""
        label = f"t_max={d.get('train_horizon', '?'):g}, b={d.get('b', '?'):g}{sd_lab}"
        series.append((ep, y, label))

    if not series:
        print("plot_training_loss_curves: no case JSON with train_total_loss_history", flush=True)
        return

    fig, ax = plt.subplots(figsize=(10, 5))
    for ep, y, label in series:
        ax.semilogy(ep, y, label=label, linewidth=0.9)
    ax.set_xlabel("epoch")
    ax.set_ylabel("train total loss (semilogy)")
    ax.set_title("Adam PINN: total training loss vs epoch")
    ax.legend(fontsize=9)
    ax.grid(True, which="both", alpha=0.35)
    fig.tight_layout()
    fig.savefig(out_dir / "train_total_loss_semilogy.png", dpi=160, bbox_inches="tight")
    plt.close(fig)

    fig2, ax2 = plt.subplots(figsize=(10, 5))
    for ep, y, label in series:
        ax2.plot(ep, np.log10(y), label=label, linewidth=0.9)
    ax2.set_xlabel("epoch")
    ax2.set_ylabel(r"log10(train total loss)")
    ax2.set_title("Adam PINN: log10(total training loss) vs epoch")
    ax2.legend(fontsize=9)
    ax2.grid(True, alpha=0.35)
    fig2.tight_layout()
    fig2.savefig(out_dir / "train_total_loss_log10.png", dpi=160, bbox_inches="tight")
    plt.close(fig2)
    print(f"Saved loss curves to {out_dir / 'train_total_loss_semilogy.png'} and ..._log10.png", flush=True)
...
def main():
    ...
    x_label = "1/b"
    regime_overlay = build_regime_overlay_from_shape(train_loss_plot.shape)
    ...
    plot_heatmap(
        ...
        nt_values,
        "Training Loss",
        out_dir / "train_loss_heatmap.png",
        x_label,
        y_label,
        boundary_data={"training_loss": train_loss_plot, "test_error": test_error_plot},
        regime_overlay=regime_overlay,
        boundary_percentile=args.boundary_percentile,
    )
    plot_heatmap(
        train_data_loss_plot,
        inv_b_sorted,
        nt_values,
        "Train Data Loss (L1)",
        out_dir / "train_data_loss_heatmap.png",
        x_label,
        y_label,
        regime_overlay=regime_overlay,
        boundary_percentile=args.boundary_percentile,
    ...
    plot_heatmap(
        train_physics_loss_plot,
        inv_b_sorted,
        nt_values,
        "Train Physics Loss (L_phys)",
        out_dir / "train_physics_loss_heatmap.png",
        x_label,
        y_label,
        regime_overlay=regime_overlay,
        boundary_percentile=args.boundary_percentile,
    ...
    plot_heatmap(
        train_total_loss_plot,
        inv_b_sorted,
        nt_values,
        "Train Total Loss",
        out_dir / "train_total_loss_heatmap.png",
        x_label,
        y_label,
        regime_overlay=regime_overlay,
        boundary_percentile=args.boundary_percentile,
    ...
    plot_heatmap(
        ...
        nt_values,
        "Test Rollout rel-L2",
        out_dir / "test_error_heatmap.png",
        x_label,
        y_label,
        boundary_data={"training_loss": train_loss_plot, "test_error": test_error_plot},
        regime_overlay=regime_overlay,
    ...
    plot_heatmap(
        test_rollout_error_plot,
        inv_b_sorted,
        nt_values,
        "Test Rollout MSE",
        out_dir / "test_rollout_error_heatmap.png",
        x_label,
        y_label,
        regime_overlay=regime_overlay,
        boundary_percentile=args.boundary_percentile,
    ...
    plot_heatmap(
        test_rollout_sse_plot,
        inv_b_sorted,
        nt_values,
        "Test Rollout Error (SSE)",
        out_dir / "test_rollout_sse_heatmap.png",
        x_label,
        y_label,
        regime_overlay=regime_overlay,
        boundary_percentile=args.boundary_percentile,
    ...
    if not args.load_results:
        plot_training_loss_curves(out_dir)

    import shutil

    phase_plot_dir = out_dir / "phase_plot"
    phase_plot_dir.mkdir(parents=True, exist_ok=True)
    for src_name, dst_name in (
        ("train_total_loss_heatmap.png", "training_loss.png"),
        ("test_error_heatmap.png", "test_rollout_l2.png"),
        ("test_error_heatmap.png", "test_error.png"),
        ("train_data_loss_heatmap.png", "train_data_loss.png"),
        ("train_physics_loss_heatmap.png", "train_physics_loss.png"),
    ):
        src = out_dir / src_name
        if src.is_file():
            shutil.copy2(src, phase_plot_dir / dst_name)
