...
**Option A — pre-generated files (recommended):**  
Download the pre-generated `.mat` files from our Zenodo archive:  
> **[TODO: insert Zenodo link after upload]**  
Place the files under `PINO/data/`.

**Option B — regenerate yourself:**
```bash
cd PINO
...
```
Supported `--r` values in the paper: 2, 4, 6, 8, 10, 16, 32, 64, 128.

**Option C — use the original piececonst benchmark (r=4 only):**  
The canonical benchmark files (`piececonst_r421_N1024_smooth1.mat`, `piececonst_r421_N1024_smooth2.mat`) are available from the [FNO repository](https://github.com/neuraloperator/neuraloperator).
...
**Pre-generated files (recommended):**  
Download from our Zenodo archive:  
> **[TODO: insert Zenodo link after upload]**  
Extract to `FNO/data/` so that paths like `FNO/data/poisson/_train_k1.0_2.5_64k.h5` are valid.
...
python utils/gen_data_helmholtz.py    # 2D Helmholtz
```
Each script writes to `./data/{poisson,advdiff,helmholtz}/`.  
Generation takes ~30–60 min per PDE on a single CPU core.
